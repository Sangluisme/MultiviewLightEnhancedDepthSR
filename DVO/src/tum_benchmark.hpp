...
cv::Mat loadDepth(const std::string &filename)
{
    ...
    cv::Mat imgDepth;
    //imgDepthIn.convertTo(imgDepth, CV_32FC1, (50.0 / 65535.0));
    imgDepthIn.convertTo(imgDepth, CV_32FC1, (1.0 / 1000.0));
    // float* const ptrIn = (float*)imgDepth.data;
    // int w = imgDepth.cols;
    // int h = imgDepth.rows;
...
bool loadPoses(const std::string &filename, std::vector<Eigen::Matrix4f> &initial_poses)
{
    if (filename.empty())
        return false;
    ...
    while (std::getline(trajFile, line))
    {
        ...
        Eigen::Quaternionf q;
        if (!(iss >> timestamp >>translation[0] >> translation [1] >> translation[2] >> q.x() >> q.y() >> q.z() >> q.w() ))
            break;  
